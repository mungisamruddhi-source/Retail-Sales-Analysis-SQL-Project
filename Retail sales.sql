...
select category, sum(total_sale) as total , count(*) as total_orders from retail_sales
group by category ;
...
select  category , count(distinct customer_id) as dist_cust from retail_sales
group by 1 ;
...
order by AVG(total_sale) DESC) as sales_rank   from retail_sales 
group by 1, 2 ) as t1 
where sales_rank = 1;
